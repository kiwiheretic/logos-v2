...
from logos.models import RoomPermissions, NetworkPermissions
...
class PrayersListView(ListView):
    template_name = "prayers/list.html"

    @method_decorator(login_required)
    def dispatch(self, request, *args, **kwargs):
        return super(PrayersListView, self).dispatch(request, *args, **kwargs)

    def get_queryset(self):
        # TODO:  Only show prayer objects that
        # are in the room the person is room_admin in
        return Prayer.objects.all()

    def get_context_data(self, **kwargs):
        context = super(PrayersListView, self).get_context_data(**kwargs)
        net_perms = get_perms(self.request.user, NetworkPermissions)
        room_perms = get_perms(self.request.user, RoomPermissions)
        context.update({'net_perms':net_perms, 'room_perms':room_perms})
        return context

class PrayerDeleteView(DeleteView):
    model = Prayer
    template_name = "prayers/confirm_delete.html"
    success_url = reverse_lazy('prayers:list')
